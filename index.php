...
if ($viewMode === '4week') {
    ...
    if ($hasWeek) {
        $windowStart = $weekMondays[0];
        $lastWeekMonday = $weekMondays[array_key_last($weekMondays)];
        $thisWeek = $windowStart;
        ...
        $pageDescription = 'Live music and events in Columbus, Ohio for ' . $weekHeader . '.';
        $canonicalUrl = CalendarSeo::pageUrl($selectedDate);
        $venueSlugs = [];

        foreach ($weekMondays as $monday) {
            $path = $weeksDir . '/' . $monday . '.json';
            if (!is_readable($path)) {
                continue;
            }

            $weekDataForWindow = WeekRenderer::loadWeek($path);
            foreach (EventClassifier::tagCountsForWeek($weekDataForWindow) as $tag => $count) {
                $tagCounts[$tag] = ($tagCounts[$tag] ?? 0) + $count;
            }

            $totalEventCount += EventClassifier::eventCountForWeek($weekDataForWindow);
            foreach ($weekDataForWindow['days'] ?? [] as $day) {
                foreach ($day['venues'] ?? [] as $venue) {
                    ...
                    $venueSlugs[$slug] = true;
                }
            }

            $fourWeekHtml .= WeekByVenueRenderer::render($weekDataForWindow, $monday);
        }

        $totalVenueCount = count($venueSlugs);
        $availableTags = array_keys($tagCounts);
    } else {
        $pageTitle = 'Columbus Live Music — ' . weekHeader($thisWeek);
        $pageDescription = 'No calendar data for the next 4 weeks.';
        $canonicalUrl = CalendarSeo::pageUrl($selectedDate);
    }
} else {
...
$viewModeNavQuery = $tagsQuery . $findQueryParam . $scopeQuery . $prefsQuery . $filterQuery;
$viewModeIconData = viewModeIconLinks($selectedDate, $viewMode, $viewModeNavQuery, 'index');
$dayHref = $viewModeIconData['dayHref'];
$weekHref = $viewModeIconData['weekHref'];
$fourWeekHref = $viewModeIconData['fourWeekHref'];
$showViewModeStrip = $hasWeek;
$viewModeVenueSlug = '';
